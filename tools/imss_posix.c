...
int close(int fd)
{
	...
	{
		slog_debug("[POSIX]. Calling Hercules 'close', pathname=%s, fd=%d.", path, fd);
		TIMING(ret = imss_close(path), "imss_close", int);
		slog_debug("[POSIX]. Ending Hercules 'close', pathname=%s, ret=%d.", path, ret);
...
int __lxstat(int fd, const char *pathname, struct stat *buf)
{
	...
	if (new_path != NULL)
	{
		slog_debug("[POSIX]. Calling Hercules '__lxstat', pathname=%s, new_path=%s", pathname, new_path);
		...
		slog_debug("[POSIX]. End Hercules '__lxstat', ret=%d, errno=%d.", ret, errno);
		free(new_path);
	}
	else
	{
...
int __xstat(int fd, const char *pathname, struct stat *buf)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX] End Hercules '__xstat', pathname=%s, fd=%d, errno=%d, ret=%d.", pathname, fd, errno, ret);
		free(new_path);
	}
	else
	{
...
int lstat(const char *pathname, struct stat *buf)
{
	...
	if (new_path != NULL)
	{
		// {
		// int exist = map_fd_search(map_fd, new_path, &ret, &p);
		slog_debug("[POSIX] Calling Hercules 'lstat'");
		imss_refresh(new_path);
		ret = imss_getattr(new_path, buf);
		slog_debug("[POSIX] Ending Hercules 'lstat'");
		free(new_path);
	}
	else
	{
...
int __xstat64(int ver, const char *pathname, struct stat64 *stat_buf)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. Ending Hercules '__xstat64', pathname=%s, ret=%d.", pathname, ret);
		free(new_path);
	}
	else
	{
...
int __open_2(const char *pathname, int flags, ...)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. Ending Hercules '__open_2'.");
		free(new_path);
	}
	else
	{
...
size_t fwrite(const void *buf, size_t size, size_t count, FILE *fp)
{
	...
	if (map_fd_search_by_val(map_fd, path, fp->_fileno) == 1)
	{
		...
		// imss_release(path);
		slog_debug("[POSIX %d]. Ending Hercules 'fwrite'., ret=%ld", ret, path);
	}
	else
	{
...
int open(const char *pathname, int flags, ...)
{
	...
	if (flags & O_CREAT)
	{
		slog_debug("[POSIX]. O_CREAT flag");
	}
	if (flags & O_TRUNC)
	{
		slog_debug("[POSIX]. O_TRUNC flag");
	}
	if (flags & O_EXCL)
	{
		slog_debug("[POSIX]. O_EXCL flag");
	}
	...
	if (new_path != NULL)
	{
		...
		if (exist == -1) // if the "new_path" was not find:
		{
			...
			{
				slog_debug("[POSIX]. New file %s", new_path);
				int err_create = imss_create(new_path, mode, &ret_ds);
			...
			else // the file must exist.
			{
				...
				TIMING(ret = imss_open(new_path, &ret_ds), "imss_open op2", int);
				slog_debug("[POSIX] 2 - ret_ds=%d, new_path=%s", ret_ds, new_path);
		...
		slog_debug("[POSIX] Ending Hercules 'open', exist=%d, mode=%d, ret=%d", exist, mode, ret);
		free(new_path);
	}
	else
	{
		...
		slog_debug("[POSIX]. Ending real 'open', mode=%d, pathname=%s, ret=%d.", mode, pathname, ret);
	}
...
off_t lseek(int fd, off_t offset, int whence)
{
	...
	{
		slog_debug("[POSIX]. Calling Hercules 'lseek', path=%s", path);
		...
		slog_debug("[POSIX]. Ending Hercules 'lseek', ret=%ld", ret);
	}
	else
	{
...
ssize_t write(int fd, const void *buf, size_t size)
{
	...
	if (map_fd_search_by_val(map_fd, path, fd) == 1)
	{
		slog_debug("[POSIX]. Calling Hercules 'write', path=%s", path);
...
ssize_t read(int fd, void *buf, size_t size)
{
	...
	if (map_fd_search_by_val(map_fd, path, fd) == 1)
	{
		...
		// ret = size+1;
		slog_debug("[POSIX]. End Hercules 'read', pathname=%s, ret=%ld, size=%ld.", path, ret, size);
		// if (ret < size)
		// 	ret = 0;
	}
	else
...
int access(const char *path, int mode)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. End Hercules 'access', path=%s, new_path=%s ret=%d, errno=%d.", path, new_path, ret, errno);
		free(new_path);
	}
	else
	{
		...
		slog_debug("[POSIX]. End Real 'access', ret=%d, errno=%d.", ret, errno);
	}
