...
int stat(const char *pathname, struct stat *buf)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. Ending Hercules 'stat', new_path=%s, errno=%d:%s", new_path, errno, strerror(errno));
		free(new_path);
	}
	else
	{
...
int ferror(FILE *fp)
{
	...
	{
		// TODO.
		slog_debug("[POSIX][TODO]. Calling Hercules 'ferror', pathname=%s", pathname);
		fprintf(stderr, "[POSIX][TODO]. Calling Hercules 'ferror', pathname=%s", pathname);
	}
	else
	{
		ret = real_ferror(fp);
...
// 	if (new_path != NULL)
// 	{
// 		slog_debug("[POSIX] Calling Hercules 'openat' flags=%d, mode=%o, pathname=%s, new_path=%s", flags, mode, pathname, new_path);
...
// 				if(pathname_dir == NULL) { // if dirfd is not storage locally
...
// 				sprintf(absolute_pathname,"%s%s", pathname_dir, );
...
int mkdir(const char *path, mode_t mode)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. Ending hercules 'mkdir', path=%s, new_path=%s\n", path, new_path);
		free(new_path);
	}
	else
	{
...
int unlink(const char *name)
{
	...
	if (new_path != NULL)
	{
		...
		ret = map_fd_erase_by_pathname(map_fd, new_path);
		if (ret == -1)
		{
...
{
	fprintf(stderr, "[POSIX][TODO]. Calling Real 'fstatat', pathname=%s", __file);
...
{
	fprintf(stderr, "[POSIX][TODO]. Calling Real 'fstatat64', pathname=%s", __file);
...
{
	fprintf(stderr, "[POSIX][TODO]. Calling Real 'newfstatat', pathname=%s", pathname);
	if (!real_newfstatat)
...
int __fxstat64(int ver, int fd, struct stat64 *buf)
{
	...
	// fprintf(stderr, "[TODO] Calling __fxstat64, fd=%d\n", fd);
