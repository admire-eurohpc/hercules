...
int __lxstat(int fd, const char *pathname, struct stat *buf)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. End Hercules '__lxstat', pathname=%s, new_path=%s, fd=%d, ret=%d, errno=%d:%s, file_size=%lu\n", pathname, new_path, fd, ret, errno, strerror(errno), buf->st_size);
		free(new_path);
	}
	else
	{
...
int __lxstat64(int fd, const char *pathname, struct stat64 *buf)
{
	...
	{
		slog_debug("[POSIX]. Calling Real '__lxstat64', pathname=%s", pathname);
		ret = real__lxstat64(fd, pathname, buf);
		slog_debug("[POSIX]. End Real '__lxstat64', pathname=%s", pathname);
	}
...
int __xstat(int ver, const char *pathname, struct stat *stat_buf)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX] End Hercules '__xstat', pathname=%s, ver=%d, new_path=%s, errno=%d:%s, ret=%d\n", pathname, ver, new_path, errno, strerror(errno), ret);
		free(new_path);
	}
	else
	{
		slog_debug("[POSIX]. Calling Real '__xstat', pathname=%s, ver=%d", pathname, ver);
		ret = real___xstat(ver, pathname, stat_buf);
		slog_debug("[POSIX]. End Real '__xstat', pathname=%s, ver=%d, errno=%d:%s, ret=%d", pathname, ver, errno, strerror(errno), ret);
	}
	return ret;
}
...
pid_t fork(void)
{
	...
	if (pid == 0) // child process.
	{
		...
		// release is set to 0 to prevent to close the communication twice.
		release = 0;
...
int stat(const char *pathname, struct stat *buf)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. Ending Hercules 'stat', new_path=%s, errno=%d:%s\n", new_path, errno, strerror(errno));
		free(new_path);
	}
	else
	{
...
extern int statvfs(const char *__restrict __file, struct statvfs *__restrict __buf)
{
	...
	{
		slog_debug("[POSIX]. Calling Real 'statvfs', path=%s", __file);
		ret = real_statvfs(__file, __buf);
		slog_debug("[POSIX]. Ending Real 'statvfs', path=%s", __file);
	}
...
int __xstat64(int ver, const char *pathname, struct stat64 *stat_buf)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. Ending Hercules '__xstat64', pathname=%s, ret=%d, errno=%d:%s\n", pathname, ret, errno, strerror(errno));
	}
	else
	{
...
char *realpath(const char *pathname, char *resolved_path)
{
	...
	// else
	{
		slog_debug("[POSIX]. Calling real 'realpath', pathname=%s", pathname);
		p = real_realpath(pathname, resolved_path);
		slog_debug("[POSIX]. Ending real 'realpath', pathname=%s, resolved_path=%s, ret=%d, errno=%d:%s\n", pathname, resolved_path, ret, errno, strerror(errno));
	}
	return p;
}
...
int open64(const char *pathname, int flags, ...)
{
	...
	{
		slog_debug("[POSIX]. Calling real 'open64', pathname=%s", pathname);
		if (!mode)
			ret = real_open64(pathname, flags);
		else
			ret = real_open64(pathname, flags, mode);
		slog_debug("[POSIX]. Ending real 'open64', pathname=%s, ret=%d, errno=%d:%s", pathname, ret, errno, strerror(errno));
	}
	return ret;
}
...
int mkstemp(char *template)
{
	...
	{
		slog_debug("[POSIX]. Calling Real 'mkstemp', pathname=%s", template);
		ret = real_mkstemp(template);
		slog_debug("[POSIX]. Ending Real 'mkstemp', pathname=%s", template);
	}
	return ret;
}
...
ssize_t writev(int fd, const struct iovec *iov, int iovcnt)
{
	...
	else
	{
		...
		slog_debug("[POSIX]. Ending real 'writev', fd=%d, errno=%d:%s", fd, errno, strerror(errno));
	}
...
int ppoll(struct pollfd *fds, nfds_t nfds, const struct timespec tmo_p, const sigset_t sigmask)
{
	...
	else
	{
		...
		// slog_debug("[POSIX]. Ending real 'ppoll', fd=%d, errno=%d:%s", fds->fd, errno, strerror(errno));
	}
	return ret;
}
...
int open(const char *pathname, int flags, ...)
{
	...
	{
		slog_debug("[POSIX]. Calling real 'open', flags=%d, mode=%o, pathname=%s", flags, mode, pathname);
		// checkOpenFlags(pathname, flags);
...
int openat(int dirfd, const char *pathname, int flags, ...)
{
	...
	{
		slog_debug("[POSIX] Calling Hercules 'openat' flags=%d, mode=%o, dirfd=%d, pathname_dir=%s, pathname=%s", flags, mode, dirfd, pathname_dir, pathname);
		...
		{
			if (dirfd == AT_FDCWD) // dirfd is the special value AT_FDCWD.
			{					   // TO CHECK!
				char *new_path = checkHerculesPath(pathname);
			...
			{
				// // get the pathname of the directory pointed by dirfd if it is storage in the local map "map_fd".
				// char *pathname_dir = map_fd_search_by_val(map_fd, dirfd);
				// if (pathname_dir == NULL)
				// { // if dirfd is not storage locally
				// 	// search dirfd on the metadata server.
				// 	// fprintf(stderr,"");
				// 	slog_error("[POSIX] dirfd=%d could not be resolved.");
				// 	return -1;
				// }
	...
	{
		slog_debug("[POSIX] Calling real 'openat', flags=%d, mode=%o, dirfd=%d, pathname=%s", flags, mode, dirfd, pathname);
		if (!mode)
		{
...
int mkdir(const char *path, mode_t mode)
{
	...
	if (new_path != NULL)
	{
		...
		if (ret < 0)
		{
			errno = -ret;
			ret = -1;
			slog_debug("[POSIX]. Error in 'mkdir', ret=%ld, errno=%d:%s", ret, errno, strerror(errno));
		}
		slog_debug("[POSIX]. Ending hercules 'mkdir', path=%s, new_path=%s, ret=%d, errno=%d:%s\n", path, new_path, ret, errno, strerror(errno));
		free(new_path);
	}
	else
	{
...
off_t lseek(int fd, off_t offset, int whence)
{
	...
	{
		slog_debug("[POSIX]. Calling Real 'lseek', fd=%d, whence=%d, offset=%ld, errno=%d:%s", fd, whence, offset, errno, strerror(errno));
		...
		slog_debug("[POSIX]. Ending Real 'lseek', fd=%d, whence=%d, offset=%ld, errno=%d:%s", fd, whence, offset, errno, strerror(errno));
...
off64_t lseek64(int fd, off64_t offset, int whence)
{
	...
	{
		unsigned long p = 0;
		slog_debug("[POSIX]. Calling Hercules 'lseek64', pathname=%s, fd=%d, errno=%d:%s", pathname, fd, errno, strerror(errno));
		slog_info("[POSIX]. whence=%d, offset=%ld", whence, offset);
		if (whence == SEEK_SET)
		{
		...
		slog_debug("[POSIX]. Ending Hercules 'lseek64', ret=%ld, errno=%d:%s\n", ret, errno, strerror(errno));
	}
	else
	{
		slog_debug("[POSIX]. Calling real 'lseek64', fd=%d, errno=%d:%s", fd, errno, strerror(errno));
...
ssize_t write(int fd, const void *buf, size_t size)
{
	...
	{
		slog_debug("[POSIX]. Calling real 'write', fd=%d, size=%lu", fd, size);
		ret = real_write(fd, buf, size);
		slog_debug("[POSIX]. Ending real 'write', fd=%d, ret=%ld", fd, ret);
	}
...
ssize_t read(int fd, void *buf, size_t size)
{
	...
	if (pathname != NULL)
	{
		...
		slog_debug("[POSIX]. End Hercules 'read', pathname=%s, ret=%ld, size=%ld, fd=%d, errno=%d:%s\n", pathname, ret, size, fd, errno, strerror(errno));
...
ssize_t pread(int fd, void *buf, size_t count, off_t offset)
{
	...
	{
		unsigned long offset = 0;
		slog_debug("[POSIX]. Calling Hercules 'pread', pathname=%s, size=%ld, fd=%ld.", pathname, count, fd);
		...
		if (ret < 0)
		{
			errno = -ret;
			ret = -1;
			slog_error("[POSIX] Error Hercules 'pread'	: %s", strerror(errno));
		}
		else if (offset >= ds_stat_n.st_size)
		{
			ret = 0;
		...
		slog_debug("[POSIX]. End Hercules 'pread', pathname=%s, ret=%ld, size=%ld, fd=%d, errno=%d:%s", pathname, ret, count, fd, errno, strerror(errno));
	}
	else
	{
		slog_debug("[POSIX]. Calling real 'pread', size=%ld, fd=%d.", count, fd);
		ret = real_pread(fd, buf, count, offset);
		slog_debug("[POSIX]. Ending real 'pread', size=%ld, fd=%d, ret=%d, errno=%d:%s.", count, fd, ret, errno, strerror(errno));
	}
	return ret;
}
...
DIR *opendir(const char *name)
{
	...
	else
	{
		...
		slog_debug("[POSIX]. Ending Real 'opendir', pathname=%s, errno=%d:%s", name, errno, strerror(errno));
	}
	return dirp;
}
...
struct dirent *readdir(DIR *dirp)
{
	...
	else
	{
		...
		slog_debug("[POSIX]. Ending real 'readdir', errno=%d:%s", errno, strerror(errno));
	}
...
struct dirent64 *readdir64(DIR *dirp)
{
	...
	if (pathname != NULL)
	{
		...
		slog_debug("[POSIX]. Ending Hercules 'readdir64', pathname=%s, errno=%d:%s\n", pathname, errno, strerror(errno));
...
int closedir(DIR *dirp)
{
	...
	if (pathname != NULL)
	{
		...
		slog_debug("[POSIX] End Hercules 'closedir', fd=%d, errno=%d:%s, ret=%d", fd, errno, strerror(errno), ret);
		// fprintf(stderr, "Hercules closedir, %d\n", ret);
	}
	else
	{
		slog_debug("[POSIX] Calling Real 'closedir'");
		ret = real_closedir(dirp);
		slog_debug("[POSIX] End Real 'closedir', errno=%d:%s, ret=%d", errno, strerror(errno), ret);
	}
...
int __fxstatat(int ver, int dirfd, const char *pathname, struct stat *stat_buf, int flags)
{
	// fprintf(stderr, "[POSIX]. Calling Real '__fxstatat', pathname=%s, dirfd=%d\n", pathname, dirfd);
	if (!real___fxstatat)
	{
	...
	if (!init)
	{
		return real___fxstatat(ver, dirfd, pathname, stat_buf, flags);
	}
	...
	if (pathname_dir != NULL || new_path != NULL)
	{
		// fprintf(stderr, "[POSIX] Calling Hercules '__fxstatat' flags=%d, dirfd=%d, pathname_dir=%s, new_path_dir=%s, pathname=%s\n", flags, dirfd, pathname_dir, new_path, pathname);
		slog_debug("[POSIX] Calling Hercules '__fxstatat' flags=%d, dirfd=%d, pathname_dir=%s, pathname=%s", flags, dirfd, pathname_dir, pathname);
		int is_absolute_path = IsAbsolutePath(pathname);
		...
		slog_debug("[POSIX] Ending Hercules '__fxstatat', ret=%d, errno=%d:%s\n", ret, errno, strerror(errno));
		if (new_path != NULL)
			free(new_path);
	}
	else
	{
		slog_debug("[POSIX] Calling real '__fxstatat' flags=%d, dirfd=%d, pathname=%s", flags, dirfd, pathname);
		// fprintf(stderr, "[POSIX] Calling real '__fxstatat' flags=%d, dirfd=%d, pathname=%s\n", flags, dirfd, pathname);
		ret = real___fxstatat(ver, dirfd, pathname, stat_buf, flags);
		slog_debug("[POSIX] Ending real '__fxstatat' flags=%d, dirfd=%d, pathname=%s, errno=%d:%s", flags, dirfd, pathname, errno, strerror(errno));
	}

	return ret;
}

int __fxstatat64(int ver, int dirfd, const char *pathname, struct stat64 *stat_buf, int flags)
{
	...
	if (!init)
	{
		return real___fxstatat64(ver, dirfd, pathname, stat_buf, flags);
	}
	...
	{
		// fprintf(stderr, "[POSIX] Calling Hercules '__fxstatat' flags=%d, dirfd=%d, pathname_dir=%s, new_path_dir=%s, pathname=%s\n", flags, dirfd, pathname_dir, new_path, pathname);
		slog_debug("[POSIX] Calling Hercules '__fxstatat64' flags=%d, dirfd=%d, pathname_dir=%s, pathname=%s", flags, dirfd, pathname_dir, pathname);
		int is_absolute_path = IsAbsolutePath(pathname);
	...
	{
		slog_debug("[POSIX] Calling real '__fxstatat64' flags=%d, dirfd=%d, pathname=%s", flags, dirfd, pathname);
		// fprintf(stderr, "[POSIX] Calling real '__fxstatat' flags=%d, dirfd=%d, pathname=%s\n", flags, dirfd, pathname);
		ret = real___fxstatat64(ver, dirfd, pathname, stat_buf, flags);
		slog_debug("[POSIX] Ending real '__fxstatat64' flags=%d, dirfd=%d, pathname=%s, errno=%d:%s", flags, dirfd, pathname, errno, strerror(errno));
	}

	return ret;
}

// int (*real_faccessat)(int dirfd, const char *pathname, int mode, int flags) = NULL;
int faccessat(int dirfd, const char *pathname, int mode, int flags)
{
	...
	if (pathname_dir != NULL || new_path != NULL)
	{
		// fprintf(stderr, "[POSIX] Calling Hercules 'faccessat' flags=%d, dirfd=%d, pathname_dir=%s, new_path_dir=%s, pathname=%s\n", flags, dirfd, pathname_dir, new_path, pathname);
		slog_debug("[POSIX] Calling Hercules 'faccessat' flags=%d, dirfd=%d, pathname_dir=%s, pathname=%s", flags, dirfd, pathname_dir, pathname);
		int is_absolute_path = IsAbsolutePath(pathname);
	...
	{
		slog_debug("[POSIX] Calling real 'faccessat' flags=%d, dirfd=%d, pathname=%s", flags, dirfd, pathname);
		// fprintf(stderr, "[POSIX] Calling real 'faccessat' flags=%d, dirfd=%d, pathname=%s\n", flags, dirfd, pathname);
		ret = real_faccessat(dirfd, pathname, mode, flags);
		slog_debug("[POSIX] Ending real 'faccessat' flags=%d, dirfd=%d, pathname=%s, errno=%d:%s", flags, dirfd, pathname, errno, strerror(errno));
	}
...
int unlinkat(int dirfd, const char *pathname, int flags)
{
	...
	if (pathname_dir != NULL || new_path != NULL)
	{
		slog_debug("[POSIX] Calling Hercules 'unlinkat' flags=%d, dirfd=%d, pathname_dir=%s, pathname=%s", flags, dirfd, pathname_dir, pathname);
		int is_absolute_path = IsAbsolutePath(pathname);
	...
	{
		slog_debug("[POSIX] Calling real 'unlinkat' flags=%d, dirfd=%d, pathname=%s", flags, dirfd, pathname);
		fprintf(stderr, "[POSIX] Calling real 'unlinkat' flags=%d, dirfd=%d, pathname=%s\n", flags, dirfd, pathname);
		ret = real_unlinkat(dirfd, pathname, flags);
		slog_debug("[POSIX] Ending real 'unlinkat' flags=%d, dirfd=%d, pathname=%s, errno=%d:%s", flags, dirfd, pathname, errno, strerror(errno));
	}
...
int __fxstat64(int ver, int fd, struct stat64 *buf)
{
	...
	{
		slog_debug("[POSIX] Calling Real '__fxstat64', ver=%d, fd=%d", ver, fd);
		ret = real__fxstat64(ver, fd, buf);
		slog_debug("[POSIX] End Real '__fxstat64', fd=%d, errno=%d:%s, ret=%d, st_size=%ld, st_blocks=%ld, st_blksize=%ld", fd, errno, strerror(errno), ret, buf->st_size, buf->st_blocks, buf->st_blksize);
	}
	// report(pathname, buf);
	// StatReport(fd, *buf);
	return ret;
...
int __fxstat(int ver, int fd, struct stat *buf)
{
	...
	if (pathname != NULL)
	{
		...
		slog_debug("[POSIX] End Hercules '__fxstat', pathname=%s, fd=%d, errno=%d:%s, ret=%d, st_size=%ld, st_blocks=%ld, st_blksize=%ld\n", pathname, fd, errno, strerror(errno), ret, buf->st_size, buf->st_blocks, buf->st_blksize);
	}
	else
	{
		slog_debug("[POSIX] Calling Real '__fxstat', ver=%d, fd=%d", ver, fd);
		ret = real__fxstat(ver, fd, buf);
		slog_debug("[POSIX] End Real '__fxstat', fd=%d, errno=%d:%s, ret=%d, st_size=%ld, st_blocks=%ld, st_blksize=%ld", fd, errno, strerror(errno), ret, buf->st_size, buf->st_blocks, buf->st_blksize);
	}
	// report(pathname, buf);
	// StatReport(fd, *buf);
	return ret;
...
int access(const char *path, int mode)
{
	...
	if (new_path != NULL)
	{
		...
		slog_debug("[POSIX]. End Hercules 'access', new_path=%s ret=%d, errno=%d:%s\n", new_path, ret, errno, strerror(errno));
...
char *getcwd(char *buf, size_t size)
{
	...
	{
		slog_debug("[POSIX] Calling real 'getcwd'");
		real_getcwd(buf, size);
		slog_debug("[POSIX] Ending real 'getcwd', buf=%s", buf);
	}
...
// 		real_posix_fadvise64 = dlsym(RTLD_NEXT, "posix_fadvise64");

// 	fprintf(stderr, "Calling 'posix_fadvise64', fd=%d, errno=%d:%s\n", fd, errno, strerror(errno));
// 	if (init)
// 	{
// 		slog_debug("Calling posix_fadvise64, fd=%d", fd);
// 	}
// 	return real_posix_fadvise64(fd, offset, len, advice);
// }
...
int syncfs(int fd)
{
	...
	if (pathname = map_fd_search_by_val(map_fd, fd))
	{
		slog_debug("[POSIX] Calling Hercules syncfs, pathname=%s", pathname);
	}
...
// 		}
// 		slog_debug("[POSIX]. Ending Hercules 'fcntl', pathname=%s, fd=%d, ret=%d, errno=%d:%s", pathname, fd, ret, errno, strerror(errno));
// 	}
// 	else
// 	{
// 		slog_debug("[POSIX]. Calling Real 'fcntl', fd=%d", fd);
// 		// cmd = FCNTL_ADJUST_CMD(cmd);
// 		// switch (cmd)
// 		// {
// 		// // case F_SETLKW:
...
// 		else
// 			ret = real_fcntl(fd, cmd, arg);
// 		slog_debug("[POSIX]. Ending Real 'fcntl', fd=%d, ret=%d, errno=%d:%s", fd, ret, errno, strerror(errno));
// 	}
