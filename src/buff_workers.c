...
void * dispatcher(void * th_argv)
{
	...
	//Counter specifying the number of currently connected clients to the IMSS.
	...
	for (;;)
	{
		...
		if (!strcmp((char *) zmq_msg_data(&client_req), "HELLO!"))
		{
		...
		//Check if the client aims to disconnect from the IMSS.
		else if (!strcmp((char *) zmq_msg_data(&client_req), "CLOSE!"))
		{
			//Reduce the number of currently connected clients to the IMSS.
			connected_clients--;
		}
		//Check if the client aims to tear down the IMSS.
		else if (!strcmp((char *) zmq_msg_data(&client_req), "TEAR_DOWN!"))
		{
			//Reduce the number of currently connected clients to the IMSS.
			connected_clients--;
