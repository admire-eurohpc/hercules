...
test_file=../tst/collective_io_multiple_datasets.csv
	...
	for clients in 64 #4 8 16 32 128 
	   do
		...
		for dataset_size in 8388608 #2097152 8388608 33554432 # 2GB 8GB & 32GB
		   do
			num_rounds=$((dataset_size / clients))
			num_rounds=$((num_rounds / dataset_size_))

			echo -e "DATASET SIZE: $dataset_size_ KB\nNUMBER_DATASETS/CLIENT: $num_rounds\nTOTAL: $dataset_size KB" >> $test_file

			for policy in HASH LOCAL #RR  BUCKETS
			   do
				   ...
				   do
					...
					for it in {0..2}
					   do
						...
						$dataset_size_ $block_size $num_rounds $4 $5 >> $test_file
