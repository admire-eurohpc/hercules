...
do
	srun --export=ALL -N 1 -w ${hosts[$i]} --exclusive $IMSS_PATH/server m --server-id=$i --stat-logfile=./metadata --port=$META_PORT --bufsize=0 2> meta_$i.log &
done
sleep 2
...
do
	srun --export=ALL -N 1 -w ${hosts[$i]} --exclusive $IMSS_PATH/server d --server-id=$i --imss-uri=imss:// --port=$DATA_PORT --bufsize=0 --stat-host=$META_NODE --stat-port=$META_PORT --num-servers=$NUM_DATA --deploy-hostfile=./data_hostfile --block-size=$BLOCK_SIZE --storage-size=$STORAGE_SIZE 2> data_$i.log &
done
sleep 2
...
mpiexec -l -n $NUM_CLIENT --ppn 1 -f ./client_hostfile \
	...
	$IOR_PATH/ior -o /mnt/imss/data.out -t 100m -b 100m -s 1 -i 3
