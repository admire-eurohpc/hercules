...
then
	mpiexec --hostfile ./client_hostfile -npernode $H_NUM_CLIENT \
        	-x LD_PRELOAD=$H_PATH/tools/libimss_posix.so \
	        ...
	        -x IMSS_DEPLOYMENT=2 \
		-x IMSS_DEBUG=all \
		-x IMSS_MALLEABILITY=$H_MALLEABILITY \
		-x IMSS_LOWER_BOUND_H_MALLEABILITY=$IMSS_LOWER_BOUND_H_MALLEABILITY \
		-x IMSS_UPPER_BOUND_H_MALLEABILITY=$IMSS_UPPER_BOUND_H_MALLEABILITY \
	        $COMMAND
else
	mpiexec -f ./client_hostfile --ppn $H_NUM_CLIENT \
	        -env LD_PRELOAD $H_PATH/tools/libimss_posix.so \
	        -env IMSS_MOUNT_POINT /mnt/imss \
	        -env IMSS_HOSTFILE $PWD/data_hostfile \
	        -env IMSS_N_SERVERS $H_NUM_DATA \
	        -env IMSS_SRV_PORT $H_DATA_PORT \
	        -env IMSS_BUFFSIZE 1 \
        	-env IMSS_BLKSIZE $H_BLOCK_SIZE \
	        -env IMSS_META_HOSTFILE $PWD/meta_hostfile \
        	-env IMSS_META_PORT $H_META_PORT \
	        -env IMSS_META_SERVERS $H_NUM_METADATA \
        	-env IMSS_STORAGE_SIZE $H_STORAGE_SIZE \
	        -env IMSS_METADATA_FILE $PWD/metadata \
        	-env IMSS_DEPLOYMENT 2 \
                -env IMSS_MALLEABILITY $H_MALLEABILITY \
                -env IMSS_LOWER_BOUND_H_MALLEABILITY 0 \
                -env IMSS_UPPER_BOUND_H_MALLEABILITY 0 \
		$COMMAND
fi
